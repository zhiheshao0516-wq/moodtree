...
MAIL_FROM_NAME = os.environ.get('MAIL_FROM_NAME', 'MoodTree')
...
CODE_HASH_SECRET = os.environ.get('CODE_HASH_SECRET', SECRET_KEY)
BUCKET = 'moodtree-1458420446'
REGION = 'ap-shanghai'
COS_HOST = f'{BUCKET}.cos.{REGION}.myqcloud.com'

_DATA_CACHE = {'value': None, 'expires': 0.0}
...
SENSITIVE_WORDS = {
    # Chinese profanity / insults (full phrases, no single-char pinyin)
    ...
    # Pinyin abbreviations - only unambiguous full forms
    'sb',
}
...
def check_sensitive(text):
    ...
    Strategy:
    - Chinese words: simple substring match (no word boundary needed, Chinese has no spaces)
    - Short pinyin like 'sb': boundary-aware, only ASCII letter neighbors block the match
    - English words: \b word boundary match
    """
    if not text:
        return False, []
    import re
    ...
    for w in SENSITIVE_WORDS:
        ...
        if has_chinese:
            # Chinese words: simple substring match
            if w in lower:
                matched.append(w)
                seen.add(w)
        elif len(w) <= 3 and w.isascii():
            # Short ASCII words (sb, nc, etc.): boundary-aware, only ASCII letters block
            for m in re.finditer(re.escape(w), lower):
                pos = m.start()
                ch_before = lower[pos - 1] if pos > 0 else ' '
                ch_after = lower[pos + len(w)] if pos + len(w) < len(lower) else ' '
                if (ch_before.isascii() and ch_before.isalpha()) or (ch_after.isascii() and ch_after.isalpha()):
                    continue  # skip inside English words like 'describe' -> 'sb'
                matched.append(w)
                seen.add(w)
                break
        else:
            # English words: use \b word boundary
            try:
...
def read_data():
    now = time.time()
    if _DATA_CACHE['value'] is not None and now < _DATA_CACHE['expires']:
    ...
    _DATA_CACHE['expires'] = now + CACHE_TTL_SECONDS
    return data
...
def write_data(data):
    ...
    _DATA_CACHE['value'] = copy.deepcopy(data)
    _DATA_CACHE['expires'] = time.time() + CACHE_TTL_SECONDS
    return True
...
# ===== User handlers =====
EMAIL_RE = re.compile(r'^[^\s@]+@[^\s@]+\.[^\s@]{2,}$', re.UNICODE)

def _normalize_identifier(body):
...
def _find_user(data, kind, identifier):
    if kind == 'phone':
        target_digits = re.sub(r'\D', '', identifier)
...
def check_phone(body):
    ...
    if error:
        return {'error': error}
    data = read_data()
    user = _find_user(data, kind, identifier)
    if user:
        return {'exists': True, 'hasPassword': bool(user.get('password')), 'methods': [m for m in ('phone', 'email') if user.get(m)]}
    return {'exists': False, 'hasPassword': False, 'methods': [kind]}
...
def handle_login(body):
    ...
    if not user:
        ...
        while any(u['id'] == uid for u in data['users']):
            uid = gen_id()
        user = {'id': uid, kind: identifier, 'nickname': nickname or '匿名旅人', 'avatar': avatar or '', 'avatarType': 'char', 'createdAt': now_str('%Y-%m-%d %H:%M:%S'), 'password': hash_password(password)}
...
def _code_hash(target, code):
    return hmac.new(CODE_HASH_SECRET.encode('utf-8'), f'{target}:{code}'.encode('utf-8'), hashlib.sha256).hexdigest()

def _send_email_code(target, code):
    if not all((SMTP_HOST, SMTP_USER, SMTP_PASS, MAIL_FROM)):
        return False, '邮件服务尚未完成配置'
    message = EmailMessage()
    message['Subject'] = 'MoodTree 登录验证码'
    message['From'] = f'{MAIL_FROM_NAME} <{MAIL_FROM}>'
    message['To'] = target
    message.set_content(f'你的 MoodTree 验证码是：{code}\n\n验证码将在10分钟后失效。如非本人操作，请忽略此邮件。')
    try:
        if SMTP_PORT == 465:
            with smtplib.SMTP_SSL(SMTP_HOST, SMTP_PORT, timeout=12, context=ssl.create_default_context()) as server:
...
def _send_tencent_sms(target, code):
    if not all((SECRET_ID, SECRET_KEY, SMS_SDK_APP_ID, SMS_SIGN_NAME, SMS_TEMPLATE_ID)):
        return False, '短信服务尚未完成配置'
    service, host, action = 'sms', 'sms.tencentcloudapi.com', 'SendSms'
    timestamp = int(time.time())
    date = time.strftime('%Y-%m-%d', time.gmtime(timestamp))
    payload = json.dumps({'PhoneNumberSet': [target], 'SmsSdkAppId': SMS_SDK_APP_ID, 'SignName': SMS_SIGN_NAME, 'TemplateId': SMS_TEMPLATE_ID, 'TemplateParamSet': [code, '10']}, separators=(',', ':'), ensure_ascii=False)
    headers = f'content-type:application/json; charset=utf-8\nhost:{host}\nx-tc-action:{action.lower()}\n'
    signed = 'content-type;host;x-tc-action'
    canonical = '\n'.join(('POST', '/', '', headers, signed, hashlib.sha256(payload.encode()).hexdigest()))
    ...
    request = urllib.request.Request(f'https://{host}', data=payload.encode(), method='POST', headers={'Authorization': authorization, 'Content-Type': 'application/json; charset=utf-8', 'Host': host, 'X-TC-Action': action, 'X-TC-Timestamp': str(timestamp), 'X-TC-Version': '2021-01-11', 'X-TC-Region': 'ap-guangzhou'})
    try:
        ...
        if statuses and statuses[0].get('Code') != 'Ok':
            return False, statuses[0].get('Message') or '短信发送失败，请稍后重试'
        return True, ''
    except Exception:
        return False, '短信发送失败，请稍后重试'

def send_sms_code(body):
    """Send a hashed one-time code to the selected account channel."""
    kind, target, error = _normalize_identifier(body)
    if error:
        return {'error': error}
    data = read_data()
    user = _find_user(data, kind, target)
    purpose = body.get('purpose', 'reset')
    if not user and purpose != 'bind':
        return {'error': '该账号未注册'}
    if purpose == 'bind' and kind != 'email':
        return {'error': '绑定功能仅支持邮箱'}
    now = time.time()
    rate = data.setdefault('verificationRate', {}).setdefault(target, {'last': 0, 'day': '', 'count': 0})
    today = now_str('%Y-%m-%d')
    if rate.get('day') != today:
        rate.update({'day': today, 'count': 0})
    if now - rate.get('last', 0) < 60:
        return {'error': '发送太频繁，请60秒后再试'}
    if rate.get('count', 0) >= 10:
        return {'error': '今日验证码发送次数已达上限'}
    code = str(random.randint(100000, 999999))
    if kind == 'email':
        sent, send_error = _send_email_code(target, code)
    else:
        sent, send_error = _send_tencent_sms(target, code)
    if not sent:
        return {'error': send_error}
    data['smsCodes'][target] = {'hash': _code_hash(target, code), 'expire': now + 600, 'attempts': 0}
...
def bind_email(body):
    ...
    data = read_data()
    if any(u.get('email', '').lower() == email and u.get('id') != uid for u in data['users']):
        return {'error': '该邮箱已绑定其他账号'}
    stored = data['smsCodes'].get(email)
    if not stored or time.time() > stored.get('expire', 0):
        return {'error': '验证码不存在或已过期'}
    if not hmac.compare_digest(stored.get('hash', ''), _code_hash(email, code)):
        return {'error': '验证码错误'}
    user = next((u for u in data['users'] if u.get('id') == uid), None)
    if not user:
        return {'error': '用户不存在'}
    user['email'] = email
    del data['smsCodes'][email]
    write_data(data)
    return {'success': True, 'user': {k: v for k, v in user.items() if k != 'password'}}

def verify_code_login(body):
    """Log in with a phone/email one-time code without exposing the code."""
    kind, target, error = _normalize_identifier(body)
    if error:
        return {'error': error}
    code = str(body.get('code', ''))
    data = read_data()
    stored = data['smsCodes'].get(target)
    if not stored or time.time() > stored.get('expire', 0):
        return {'error': '验证码不存在或已过期'}
    ...
    if not hmac.compare_digest(stored.get('hash', ''), _code_hash(target, code)):
        write_data(data)
        return {'error': '验证码错误'}
    user = _find_user(data, kind, target)
    if not user:
...
def _auto_reply_to_post(post):
    """Generate an AI warm reply comment after a post is created."""
    content = post.get('content', '')
    title = post.get('title', '')
    need = post.get('need', '')
    text_for_ai = f"标题：{title}\n内容：{content}" if title else content
    system_prompt = (
        '你是MoodTree情绪树洞的温暖守护者。用户刚刚分享了一篇心事，你要以匿名树友的身份留下一条温暖的评论。\n'
        '要求：\n'
        '1. 回复简短温暖，2-3句话即可，像一个善意的陌生人留的言\n'
        '2. 共情用户的感受，不说教不给建议\n'
        '3. 如果用户需要陪伴，表达"我在"；如果需要发泄，表达理解\n'
        '4. 如果涉及自伤自杀倾向，温和建议拨打心理热线400-161-9995\n'
        '5. 不要用"你好"开头，直接切入情感回应\n'
        '6. 每次回复要有变化，不要模板化'
    )
    messages = [
        {'role': 'system', 'content': system_prompt},
        {'role': 'user', 'content': f"我分享的心事：{text_for_ai}\n我的需求：{need}"}
    ]
    req_data = json.dumps({'model': 'deepseek-chat', 'messages': messages, 'max_tokens': 300, 'temperature': 0.9}).encode('utf-8')
    req = urllib.request.Request(
    ...
    try:
        ...
        return _ai_fallback(text_for_ai[:200])
...
def create_post(body):
    ...
    if has_bad:
        ...
        write_data(data)
        if ban_info:
            return {'error': f'您的内容包含敏感词汇，请修改后发布。这是您第{v_count}次违规，已被系统封禁{ban_info["label"]}。', 'matched_words': matched}
        remaining = 3 - v_count
        return {'error': f'您的内容包含敏感词汇，请修改后发布。这是您第{v_count}次违规，累计3次将被封禁（还剩{remaining}次）。', 'matched_words': matched}
    data = read_data()
    post = {
    ...
    # Auto AI reply
    try:
        reply_text = _auto_reply_to_post(post)
        if reply_text:
            ai_comment = {
            ...
            # Re-save with the AI comment
            write_data(data)
    except Exception:
        pass  # Don't fail post creation if AI reply fails
...
def add_comment(pid, body):
    ...
    for p in data['posts']:
        if p['id'] == pid:
            c = {'id': str(int(time.time()*1000)), 'author': body.get('author','匿名'), 'authorId': body.get('authorId',''), 'avatar': body.get('avatar',''), 'avatarType': body.get('avatarType','char'), 'text': body.get('text',''), 'time': '刚刚', 'created_at': int(time.time()*1000), 'likes': 0, 'replies': []}
            p['comments'].append(c)
            write_data(data)
...
def ai_chat(body):
    ...
    req_data = json.dumps({'model': 'deepseek-chat', 'messages': messages, 'max_tokens': 300, 'temperature': 0.8, 'frequency_penalty': 0.5}).encode('utf-8')
...
def main_handler(event, context):
    ...
    try:
        ...
        elif path == '/api/auth/bind-email' and method == 'POST':
            result = bind_email(req)
        elif path == '/api/sync' and method == 'GET':
            result = get_sync(qp.get('userid', ''))
        elif path == '/api/sync' and method == 'POST':
            result = save_sync(req)
